...
class GdThumb extends ThumbBase
{
	...
	{
		$validFormats = array('GIF', 'JPG', 'JPEG', 'PNG','jpg','png','jpeg','gif');
		...
		switch ($format) 
		{
			...
			case 'JPG':
				...
				break;
			case 'PNG':
				imagepng($this->oldImage, $fileName,5);
				break;
		}
	...
	public function save_watermark ($fileName, $format = null)
	{
        ...
        $image_width = $this->currentDimensions['width'];
        $image_height = $this->currentDimensions['height'];
        // echo "<pre>";
       // print_r($this);
	   // die;
        $val_format = $this->format;
        $picture_path = 'http://'.$_SERVER['HTTP_HOST'].$GLOBALS['base_path'];
        $path_watermark = $picture_path.'sites/default/files/pictures/watermark.png';
        $this->watermark_images($this->oldImage,$fileName, $path_watermark, $image_width, $image_height, 3, 85, $val_format);

		return $this;
	}
    public function watermark_images($main_img,$filename, $watermark_img, $image_width, $image_height, $padding, $quality, $format){
		
        $watermark 	= imagecreatefrompng($watermark_img); // create watermark

        if(!$main_img || !$watermark) die("Error: main image or watermark could not be loaded!");

        imagealphablending($main_img, 1);
        ...
        imagecopy($main_img, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height);
        if( strtoupper($format) == 'GIF'){
            header("content-type: image/jpeg");
            imagegif($main_img, $filename);
        }elseif(strtoupper($format) == 'JPG'){
            header("content-type: image/jpeg");
            imagejpeg($main_img, $filename,  $quality);
        }elseif(strtoupper($format) == 'PNG'){
            header("content-type: image/jpeg");
            imagepng($main_img, $filename, 5);
        }elseif(strtoupper($format) == 'JPEG'){
            header("content-type: image/jpeg");
            imagejpeg($main_img, $filename, $quality);
        }
		@chmod(dirname($fileName), 0777);
	...
	protected function determineFormat ()
	{
		...
		if( arg(0) == 'fanpage' &&  $formatInfo == ''){
			$formatInfo = getimagesize(DRUPAL_ROOT.'/sites/default/files/pictures/fanpage/likeyou_no_image.png');
