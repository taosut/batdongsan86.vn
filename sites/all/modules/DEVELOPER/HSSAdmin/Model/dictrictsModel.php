...
* @Version	 : 1.0
*/
class _Dictricts extends Dictricts{
	...
	function listItemPost(){
		...
		$header = array(
				...
				array('field' => 'i.title','data' => t('Quận/Huyện')),
				array('field' => 'i.provice_id','data' => t('Thuộc tỉnh')),
		...
		if(isset($_GET['sort'])){
			...
			$result = $sql->limit(SITE_RECORD_PER_PAGE)->orderBy('i.id', 'ASC')->execute();
		}
		...
		if($totalItem > 0){
			...
			foreach ($arrItem as $row){
				...
				$rows[$i]['data']['provice_id'] = $row->title_provice;
				$rows[$i]['data']['created'] = $created;
				$rows[$i]['data']['order_no'] =  $row->order_no;
				$rows[$i]['data']['status'] =  $status;

				$rows[$i]['data']['action'] = '<a class="icon huge" href="'.$base_url.'/admincp/dictricts/edit/'.$row->id.'"  title="Update Item">
	...
	function listInputForm(){
		...
		$arrOptionsCategory = $clsProvices->get_all_provices();
		
		$arr_fields = array(
				'id'=>array('type'=>'hidden', 'label'=>'', 'value'=>'0','require'=>'', 'attr'=>''),
				'provice_id'=>array('type'=>'option', 'label'=>'Tỉnh/Thành', 'value'=>'0','require'=>'', 'attr'=>'','list_option'=>$arrOptionsCategory),
				'title'=>array('type'=>'text', 'label'=>'Quận/Huyện', 'value'=>'','require'=>'require', 'attr'=>''),
