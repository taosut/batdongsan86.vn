...
class _Info extends Info{
	...
	function listItemPost(){
		...
		$header = array(
				...
				array('field' => 'i.keyword','data' => t('Từ khóa')),
		...
		$sql->addField('i', 'keyword', 'keyword');
		...
		if($totalItem > 0){
			...
			foreach ($arrItem as $row){
				...
				$rows[$i]['data']['keyword'] = $row->keyword;
				$rows[$i]['data']['created'] = $created;
				$rows[$i]['data']['order_no'] =  $row->order_no;
				$rows[$i]['data']['status'] =  $status;

				$rows[$i]['data']['action'] = '<a class="icon huge" href="'.$base_url.'/admincp/info/edit/'.$row->id.'"  title="Update Item">
	...
	function listInputForm(){
		...
		$arr_fields = array(
				...
				'keyword'=>array('type'=>'text', 'label'=>'Keyword', 'value'=>'','require'=>'require', 'attr'=>''),
